...
import type { TagProps, ButtonType, FormInstance, ButtonProps } from 'element-plus'
...
export interface TableRenderPublicInstance extends ComponentPublicInstance {
    $attrs: {
        renderValue: any
        renderRow: TableRow
...
export interface TableBefore {
    ...
    // 表单提交前
    onSubmit?: ({ formEl, operate, items }: { formEl: FormInstance | undefined; operate: string; items: Recordable }) => boolean | void
    // 表格内事件响应前
    onTableAction?: ({ event, data }: { event: string; data: Recordable }) => boolean | void
    // 表格顶部菜单事件响应前
    onTableHeaderAction?: ({ event, data }: { event: string; data: Recordable }) => boolean | void
    // 表格初始化前
    mount?: () => boolean | void
...
/* BaTable后置处理函数(后置埋点) */
export interface TableAfter {
    // 表格数据请求后
    getIndex?: ({ res }: { res: IResponse }) => void
    // 删除请求后
    postDel?: ({ res }: { res: IResponse }) => void
    // 编辑表单请求后
    requestEdit?: ({ res }: { res: IResponse }) => void
    // 双击单元格操作执行后
    ...
    // 表单提交后
    onSubmit?: ({ res }: { res: IResponse }) => void
    // 表格事件响应后
    onTableAction?: ({ event, data }: { event: string; data: Recordable }) => void
    // 表格顶部事件菜单响应后
    onTableHeaderAction?: ({ event, data }: { event: string; data: Recordable }) => void
    [key: string]: Function | undefined
}

export interface ComSearch {
    form: Recordable
...
/* 表格行 */
export interface TableRow extends Recordable {
    children?: TableRow[]
}

/* 表头支持的按钮 */
export type HeaderOptButton = 'refresh' | 'add' | 'edit' | 'delete' | 'unfold' | 'recycle bin' | 'comSearch' | 'quickSearch' | 'columnDisplay'
...
/* 链接打开方式 */
export type aTarget = '_blank' | '_self'

/* 通用搜索事件返回的Data */
export interface comSearchData {
    field: string
    val: string
    operator: string
    render?: string
...
export type RecordableTSX = Recordable
