...
import { Table, TableForm, TableRow, TableBefore, TableAfter, ComSearch, comSearchData, TableSchema } from '@/components/Table/types'

export const useTable = (props) => new BaseTable(props)

export class BaseTable {
    // API实例
    public api

    /* 表格状态-s 属性对应含义请查阅 Table 的类型定义 */
    ...
    /* 表单状态-s 属性对应含义请查阅 TableForm 的类型定义 */
    ...
    // Table前置处理函数列表（前置埋点）
    public before: TableBefore

    // Table后置处理函数列表（后置埋点）
    public after: TableAfter
    ...
    constructor({ api, table, form = {}, before = {}, after = {}}: TableSchema) {
        this.api = api
        ...
        this.initComSearch(!isUnDef(route) ? route.query : {})
    }
    ...
    // 复制
    requestCopy = (id: string) => {
        if (this.runBefore('requestCopy', { id }) === false) return
        this.form.loading = true
        return this.api
            .copy({
                [this.table.pk!]: id,
            ...
            .then((res) => {
                this.form.items = res.data
                this.runAfter('requestCopy', { res })
            })
            .catch((err) => {
                this.toggleForm()
                this.runAfter('requestCopy', { err })
            })
            .finally(() => {
                this.form.loading = false
            })
    ...
    // 默认
    requestDefault = (operate, id: string) => {
        if (this.runBefore('requestDefault', { id }) === false) return
        this.form.loading = true
        return this.api
            .default({
                operate,
            ...
            .then((res) => {
                this.form.items = res.data
                this.runAfter('requestDefault', { res })
            })
            .catch((err) => {
                this.toggleForm()
                this.runAfter('requestDefault', { err })
            })
            .finally(() => {
                this.form.loading = false
            })
    ...
    // switchForm = (formKey) => {
    //     if (this.runBefore('switchForm', { formKey }) === false) return
    //     this.form
    // }
    ...
    toggleForm = (operate = '', operateIds: string[] = []) => {
        ...
        if (operate == 'edit') {
            ...
            this.requestCopy(operateIds[0])
        } else if (operate !== '') {
            this.requestDefault(operate, operateIds[0])
        }
        // 开启、关闭状态由是否有操作传入来定义
        ...
        // 开启或关闭表单弹窗
        if (this.form.ref && typeof this.form.ref.open == 'function' && typeof this.form.ref.close == 'function') {
            this.form.ref[operate == '' ? 'close' : 'open'](operate, operateIds)
        }
    ...
    /**
     * 提交表单
     * @param formRef 表单组件ref
     */
    onSubmit = (formRef: ComponentRef<ElRef>) => {
        if (this.runBefore('onSubmit', { formRef: formRef, operate: this.form.operate!, items: this.form.items! }) === false) return
            ...
            this.api
                ...
                .then((res) => {
                    ...
                    if (this.form.operateIds!.length > 0) {
                        this.toggleForm(this.form.operate!, this.form.operateIds)
                    } else {
                        this.toggleForm()
                    }
     ...
     * @param data 携带数据
     */
    onTableAction = (event: string, data: Recordable) => {
        ...
        const actionFun = new Map([
            ...
            [
                'copy',
                () => {
                    this.toggleForm('copy', [data.row[this.table.pk!]])
                },
            ],
            [
                'delete',
     ...
     * @param data 携带数据
     */
    onTableHeaderAction = (event: string, data: Recordable) => {
    ...
    dragSort = () => {
        ...
        console.log('drag sort 函数待完善', el)
    }
     ...
     * @param query 要搜索的数据
     */
    initComSearch = (query: Recordable = {}) => {
